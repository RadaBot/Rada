...
class RadaReminder {
    ...
    async create({ ... data }) {
        ...
        new schedule.CronJob(data.date, async () => {
            ...
            data.user.send({
                embeds: [data.embed.setDescription(`You asked me <t:${parseInt(data.date.getTime() / 1000)}:R> to remind you of the following:\n\n${this.client.Util.trimString(data.reminder, 1800)}`)]
            })
            .catch(() => {
                data.message.reply({
                    content: data.user.toString(),
                    embeds: [data.embed.setDescription(`I tried to DM you your reminder, but I was unable to.\n\nYou asked me <t:${parseInt(data.date.getTime() / 1000)}:R> ago to remind you of the following: **${this.client.Util.trimString(data.reminder, 1800)}**`)],
                    ephermal: true
                });
            })
        }).start();
