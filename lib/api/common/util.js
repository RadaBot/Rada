...
const { devGuild } = require('../../../src/config')
...
class util {
        ...
        try {
            ...
            if (guilds.length > 0 || userId === client.user.id) return {
                ...
                user: {
                    ...
                    banner: fetched.bannerURL({ dynamic: true, size: 512 }) ?? fetched.hexAccentColor,
            ...
            return {
                ...
                user: {
                    ...
                    banner: 'https://cdn.br4d.vip/tCJr.png'
                }
            }
        }
    ...
    static addFilterWord = async (client, guildId, word, auth) => {
        ...
        if (typeof word === 'object') {
            await client.settings.set(guildId, 'filteredwords', word)
            return true
        } else {
            await client.databaseHandler.addFilterWord(guildId, word)
            return true;
        }
        // } else {
        //     return {
    ...
    static submitPreset = async (client, requester, name, preset, createdAt) => {
        let requestId = client.snowflake.generate();
        let guild = client.guilds.cache.get(devGuild);
        let channel = guild.channels.cache.get('1016618655397728347');
        channel?.send({
            content: `New preset request "${name}" from \`${requester.tag}\`\n\nWords: ${JSON.parse(preset).map(w => { return `||${w}||`}).join(', ')}\n\nThe ID for this request is \`${requestId}\``
        })
        let request = {
            id: requestId, name, requester, preset: JSON.parse(preset), createdAt
        }
        let requests = await client.settings.get(client.user.id, 'presetrequests', [])
        await client.settings.set(client.user.id, 'presetrequests', [request, ...requests])
        return true
    }

    static getPresets = async (client) => {
        let presets = await client.settings.get(client.user.id, 'presets', [])
        return presets
    }
    ...
    static deleteWarning = async (guild, user, id, client) => {
        ...
        let updated = await client.settings.get(key, 'warnings', []);
        return updated;
    }
    ...
    static clearToDoList = async (userId, client, id = null) => {
        if (!id) {
            ...
            return await client.settings.get(userId, 'todolist', [])
        } else {
            let todolist = await client.settings.get(userId, 'todolist', [])
