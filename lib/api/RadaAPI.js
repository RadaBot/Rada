...
class RadaAPI extends DefaultRouter {
    ...
    setup() {
        ...
        api.post('/preset/submit', async (req, res) => {
            let { requester, name, preset, createdAt } = req.body
            let requested = await util.submitPreset(rada, requester, name, preset, createdAt)
            res.status(200).send(JSON.stringify(requested))
        })
        api.get('/presets', async (req, res) => {
            let presets = await util.getPresets(rada)
            res.status(200).send(JSON.stringify(presets))
        })
        ...
        api.post('/updatewordfilter', async (req, res) => {
            let guildID = req.query.id;
