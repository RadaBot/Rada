...
class SlashHandler {
    ...
    async loadCommands() {
        ...
        try {
            if (production) {
                this.client.classLoader.push("[ClassLoader] SlashHandler loaded in production mode")
                return await rest.put(Routes.applicationCommands(this.client.user.id), {
                    body: this.commands
                })
            } else {
                let commands = false
                this.client.classLoader.push(`[ClassLoader] SlashHandler loaded in dev mode with commands ${commands ? "ENABLED" : "DISABLED"}`)
                return await rest.put(Routes.applicationGuildCommands(this.client.user.id, devGuild), {
                    body: commands ? this.commands : []
                })
            }
        } catch (error) {
